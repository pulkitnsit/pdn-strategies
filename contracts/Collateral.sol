...
import "./interfaces/ILpmaxiCallee.sol";
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            ILpmaxiPriceOracle(lpmaxiPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Lpmaxi: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Lpmaxi: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Lpmaxi: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Lpmaxi: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        require(redeemAmount <= totalBalance, "Lpmaxi: INSUFFICIENT_CASH");

        // optimistically transfer funds
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            ILpmaxiCallee(redeemer).lpmaxiRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Lpmaxi: INSUFFICIENT_REDEEM_TOKENS"
        );
