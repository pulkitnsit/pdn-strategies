...
import "./interfaces/ILpmaxiPriceOracle.sol";
...
contract Factory is IFactory {
    ...
    ILpmaxiPriceOracle public lpmaxiPriceOracle;
    ...
    constructor(
        ...
        ILpmaxiPriceOracle _lpmaxiPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        lpmaxiPriceOracle = _lpmaxiPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Lpmaxi: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Lpmaxi: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Lpmaxi: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Lpmaxi: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            lpmaxiPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) lpmaxiPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Lpmaxi: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Lpmaxi: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Lpmaxi: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Lpmaxi: UNAUTHORIZED");
